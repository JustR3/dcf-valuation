...
# Default values
DEFAULT_EQUITY_RISK_PREMIUM: float = 0.055  # 5.5% US ERP
...
class DamodaranLoader:
    ...
    def get_sector_priors(self, sector: str) -> SectorPriors:
        ...
        if damodaran_sector is None:
            print(
                f"⚠️  Sector '{sector}' not mapped to Damodaran dataset. "
        ...
        try:
            ...
            if self._beta_cache is not None and self._margin_cache is not None:
                ...
                print(
                    f"⚠️  Cache not available for {sector}, using generic priors"
                )
                return self._get_generic_priors(sector)

        except Exception as e:
            print(
                f"⚠️  Failed to parse Damodaran data for {sector}: {e}. "
    ...
    def _refresh_cache(self) -> None:
        ...
        try:
            print(f"   Downloading betas from {self.URL_BETAS}")
        ...
        try:
            print(f"   Downloading margins from {self.URL_MARGINS}")
    ...
    def _parse_sector_data(self, sector: str, damodaran_sector: str) -> SectorPriors:
        ...
        if self._beta_cache is not None:
            ...
            if not beta_row.empty:
                ...
                print(
                    f"   ✅ Found {sector} beta: {beta:.2f} (unlevered: {unlevered_beta:.2f})" if unlevered_beta else f"   ✅ Found {sector} beta: {beta:.2f}"
                )
            else:
                print(f"   ⚠️  No beta data found for '{damodaran_sector}'")
        ...
        if self._margin_cache is not None:
            ...
            if not margin_row.empty:
                ...
                print(f"   ⚠️  No margin data found for '{damodaran_sector}'")
        ...
        if beta is not None or operating_margin is not None:
            ...
            print(
                f"   ⚠️  No Damodaran data found for {sector}, using generic priors"
            )
            return self._get_generic_priors(sector)
