...
def main():
    ...
    parser.add_argument("command", nargs='?', choices=["valuation", "val", "dcf", "portfolio", "port", "compare"],
                       help="Command to run (omit for interactive mode)")
    parser.add_argument("tickers", nargs="*", help="Stock ticker(s)")

    # DCF-specific flags
    parser.add_argument("-g", "--growth", type=float, help="Annual growth rate (percent)")
    parser.add_argument("-t", "--terminal-growth", type=float, default=2.5,
                       help="Terminal growth rate (percent, default: 2.5)")
    parser.add_argument("-w", "--wacc", type=float, help="WACC (percent)")
    ...
    parser.add_argument("--method", type=str, choices=["max_sharpe", "min_volatility", "equal_weight"],
                       default="max_sharpe", help="Portfolio optimization method")
    ...
    if args.command in ("portfolio", "port"):
        if not args.tickers:
            ...
            if result:
                ...
                # Get the optimizer to check for errors
                ...
                error_msg = test_engine._last_error if hasattr(test_engine, '_last_error') and test_engine._last_error else "Unknown error"
                print_msg(f"Optimization failed: {error_msg}", "error")
                sys.exit(1)
        return

    # Handle 'compare' as standalone command (alias for valuation --compare)
    if args.command == "compare":
        if not args.tickers or len(args.tickers) < 2:
            print_msg("Compare command requires at least 2 tickers", "error")
            sys.exit(1)
        args.compare = True  # Set compare flag
        args.command = "valuation"  # Route to valuation handler
